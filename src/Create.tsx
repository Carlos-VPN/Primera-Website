...
const CreateAnimal: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("http://localhost:3000/create/animals", {
        method: "POST",
        body: data,
      });

      if (!response.ok) throw new Error("Error al registrar el animal");
      navigate('/index');
      alert("Animal registrado exitosamente");
      setFormData({ name: "", species: "", age: "", status: true, img: "" });
      setFile(null);
    } catch (error) {
  ...
  return (
    <div className="form-container">
      <div className="form-header">
        ...
        <p>Completa los datos para añadir un nuevo animal al refugio</p>
      </div>
